...
def compute(options):
	...
	F11,R11 = errors.jackknife('ii_plus_projected', cat1, cat1, cat1_sym, cat1_sym, options) 
	...
	export_array('%s/IIplus_proj_var_11%s.txt'%(options['2pt']['savedir'], suffix), rbins, F11, R11)

	if splitflag:
		print('22')
		F22,R22 = errors.jackknife('ii_plus_projected', cat2, cat2, cat2_sym, cat2_sym, options, rbins=rbins)
	
		print('12')
		F12,R12 = errors.jackknife('ii_plus_projected', cat1, cat2, cat1_sym, cat2_sym, options, rbins=rbins)
		print('21')
		F21,R21 = errors.jackknife('ii_plus_projected', cat2, cat1, cat2_sym, cat1_sym, options, rbins=rbins)

		export_array('%s/IIplus_proj_var_22%s.txt'%(options['2pt']['savedir'], suffix), rbins, F22, R22)
		export_array('%s/IIplus_proj_var_12%s.txt'%(options['2pt']['savedir'], suffix), rbins, F12, R12)
		export_array('%s/IIplus_proj_var_21%s.txt'%(options['2pt']['savedir'], suffix), rbins, F21, R21)

		print('00')
		cat0 = data
		F00,R00 = errors.jackknife('ii_plus_projected', cat0, cat0, cat0_sym, cat0_sym, options, rbins=rbins, period=period[options['simulation']])
		export_array('%s/IIplus_proj_var_00%s.txt'%(options['2pt']['savedir'], suffix), rbins, F00, R00)
