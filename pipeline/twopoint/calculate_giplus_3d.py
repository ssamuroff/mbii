...
def compute(options):
	...
	avec = np.vstack((data['a1'], data['a2'], data['a3'])).T
	pvec1 = np.vstack((data['x'], data['y'], data['z'])).T
	evec = np.sqrt(data['e1']*data['e1'] + data['e2']*data['e2'])

	rbins = np.logspace(np.log10(options['2pt']['rmin']), np.log10(options['2pt']['rmax']))

	import pdb ; pdb.set_trace()

	result_1 = gi_plus_3d(pvec1, avec, evec, pvec1, rbins, period=100, num_threads=1)
