...
def construct_random_cat(data, mask=None, format='halotools'):
	"""Construct a catalogue of random points drawn from the same volume as the data provided."""
	if mask is None:
		mask = np.ones(data.size).astype(bool)
	rx = (np.random.random(size=data['x'][mask].size) - 0.5) * (data['x'][mask].max()-data['x'][mask].min()) + data['x'][mask].mean()
	ry = (np.random.random(size=data['x'][mask].size) - 0.5) * (data['y'][mask].max()-data['y'][mask].min()) + data['y'][mask].mean()
	rz = (np.random.random(size=data['x'][mask].size) - 0.5) * (data['z'][mask].max()-data['z'][mask].min()) + data['z'][mask].mean()
